...
#define inc_counter_np(counter) \
  ...
  void copy_memory(bool is_aligned, Register s, Register d,
    ...
    switch (granularity) {
      case 1 :
        ld_arr = (copy_insn)&MacroAssembler::lb;
        st_arr = (copy_insn)&MacroAssembler::sb;
        break;
      case 2 :
        ld_arr = (copy_insn)&MacroAssembler::lh;
  ...
  address generate_checkcast_copy(const char *name, address *entry,
    ...
    { Label L;
      ...
      __ lw(start_to, Address(ckval, sco_offset));
  ...
  address generate_generic_copy(const char *name,
    ...
    __ add(scratch_length, length, zr);    // length (elements count, 32-bits value)
    // i.e. sign bit set
  ...
  __ BIND(L_copy_bytes);
    ...
    __ add(count, scratch_length, zr); // length
    __ j(RuntimeAddress(byte_copy_entry));
  ...
  __ BIND(L_checkcast_copy);
    // live at this point:  scratch_src_klass, scratch_length, t2 (dst_klass)
    {
      ...
      __ add(to, t0, dst);
      __ add(to, to, arrayOopDesc::base_offset_in_bytes(T_OBJECT));
      __ add(count, length, zr);           // length (reloaded)
  ...
  void generate_large_array_equals_loop(int loopThreshold,
    ...
    Label LOOP; {
      __ bind(LOOP);
      __ lw(tmp1, Address(a1, 0));
      __ add(a1, a1, wordSize);
      __ lw(tmp2, Address(a2, 0));
  ...
  void generate_compare_long_strings() {
    StubRoutines::riscv32::_compare_long_string_LL = generate_compare_long_string_same_encoding(true);
    StubRoutines::riscv32::_compare_long_string_UU = generate_compare_long_string_same_encoding(false);
    StubRoutines::riscv32::_compare_long_string_LU = generate_compare_long_string_different_encoding(true);
    StubRoutines::riscv32::_compare_long_string_UL = generate_compare_long_string_different_encoding(false);
  }
...
#define __ masm->
  ...
  address generate_throw_exception(const char* name,
    ...
    // n.b. riscv32 asserts that frame::arg_reg_save_area_bytes == 0
  ...
  void generate_all() {
    ...
    if (!UseSimpleArrayEquals) {
      StubRoutines::riscv32::_large_array_equals = generate_large_array_equals();
    }
    ...
    StubRoutines::riscv32::set_completed();
  }
