...
void MacroAssembler::arrays_equals(Register a1, Register a2, Register tmp3,
  ...
  if (UseSimpleArrayEquals) {
    ...
    add(tmp3, a1, cnt1);
    lw(tmp3, Address(tmp3, 0));
    add(tmp4, a2, cnt1);
    lw(tmp4, Address(tmp4, 0));
    xorr(tmp5, tmp3, tmp4);
    bnez(tmp5, DONE);
    j(SAME);
    bind(SHORT);
    ...
    xorr(tmp5, tmp3, tmp4);
    bnez(tmp5, DONE);
    RuntimeAddress stub = RuntimeAddress(StubRoutines::riscv32::large_array_equals());
    ...
    slli(tmp5, cnt1, 3 + log_elem_size);
    sub(tmp5, zr, tmp5);
    lw(tmp3, Address(a1, base_offset));
    lw(tmp4, Address(a2, base_offset));
    bind(LAST_CHECK);
    xorr(tmp4, tmp3, tmp4);
    sll(tmp5, tmp4, tmp5);
    bind(CSET_EQ);
